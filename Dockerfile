...
FROM bellsoft/liberica-runtime-container:jdk-all-21.0.8_12-cds-musl AS jre-builder

# Required for jlink --strip-debug to work
RUN apk add binutils

COPY --from=jar-builder /app/jtt808-server/target/jtt808-server-*.jar runner.jar
...
COPY --from=jar-builder --chown=spring:spring /app/jtt808-server/target/jtt808-server-*.jar /app/runner.jar
